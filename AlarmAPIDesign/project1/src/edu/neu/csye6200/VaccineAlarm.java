...
public class VaccineAlarm extends Alarm{
	...
	public VaccineAlarm(int DOByear,int DOBmonth,int DOBday, int intervalMonths, int frequence) {
		...
		this.setDescription("Need to vaccinate");
...
//		this.addPastEventDates();
	...
	public VaccineAlarm(int DOByear,int DOBmonth,int DOBday, int intervalMonths, int frequence, String title, String des) {
		super();
		this.dob =  LocalDate.of(DOByear, DOBmonth, DOBday);
		this.frequence = frequence;
		this.intervalMonths = intervalMonths;
	...
	public void addEventDates(){
		...
		for(int i=1;i<=frequence;i++) {
			...
			}else this.getOverDuedList().add(sDate);
		}
	}
	
	public static void demo() {
		Alarm vaAlarm = new VaccineAlarm(2017, 12, 28, 3, 3);
		vaAlarm.alarm();
	}
